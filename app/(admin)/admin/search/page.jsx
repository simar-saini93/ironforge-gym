...
function SearchContent() {
  ...
  const total = results
    ? (results.members?.length || 0) + (results.trainers?.length || 0) + (results.leads?.length || 0) + (results.reports?.length || 0)
    : 0;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => router.back()}
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Outfit', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--if-muted)', padding: 0 }}
        ...
        <>
          ...
          <Section icon={User} iconColor="var(--if-accent)" title="Members" count={results.members?.length}>
            {(results.members || []).map((m) => {
              ...
              return (
                <ResultRow key={m.id} onClick={() => router.push(`/admin/members/${m.id}`)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--if-accentbg2)', border: '1px solid var(--if-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 700, color: 'var(--if-accent)', flexShrink: 0 }}>{ini}</div>
                    <div>
                      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--if-text)' }}>{name}</p>
                      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'var(--if-muted)' }}>{m.email}</p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: 'var(--if-muted)' }}>{m.member_number}</span>
                    <Badge variant={m.sub_status === 'active' ? 'active' : 'expired'} size="sm">{m.sub_status || 'No plan'}</Badge>
                  </div>
                </ResultRow>
              );
            })}
          ...
          <Section icon={Dumbbell} iconColor="#a78bfa" title="Trainers" count={results.trainers?.length}>
            {(results.trainers || []).map((t) => {
              ...
              const ini  = name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2);
              return (
                <ResultRow key={t.id} onClick={() => router.push(`/admin/trainers/${t.id}`)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(167,139,250,0.12)', border: '1px solid #a78bfa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 700, color: '#a78bfa', flexShrink: 0 }}>{ini}</div>
                    <div>
                      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--if-text)' }}>{name}</p>
                      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'var(--if-muted)' }}>{t.specialization || t.email}</p>
                    </div>
                  </div>
          ...
          <Section icon={UserPlus} iconColor="#38bdf8" title="Leads" count={results.leads?.length}>
            {(results.leads || []).map((l) => {
              ...
              return (
                <ResultRow key={l.id} onClick={() => router.push(`/admin/crm/${l.id}`)}>
                  <div>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--if-text)' }}>{name}</p>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: 'var(--if-muted)' }}>{l.phone || l.email}</p>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Badge variant="yellow" size="sm">{l.source?.replace('_', ' ')}</Badge>
                    <Badge variant={l.status === 'new' ? 'blue' : 'yellow'} size="sm">{l.status}</Badge>
                  </div>
                </ResultRow>
              );
            })}
          ...
          <Section icon={BarChart2} iconColor="#22c55e" title="Reports" count={results.reports?.length}>
            {(results.reports || []).map((r) => (
              <ResultRow key={r.tab} onClick={() => router.push(`/admin/reports?tab=${r.tab}`)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(34,197,94,0.09)', border: '1px solid #22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <BarChart2 size={14} style={{ color: '#22c55e' }} />
                  </div>
                  <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--if-text)' }}>{r.label}</p>
                </div>
                <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 700, color: '#22c55e' }}>View →</span>
              </ResultRow>
            ))}
          </Section>
        </>
