...
function NavItem({ item, active, onClick }) {
  ...
  return (
    ...
    >
      <Icon size={18} style={{ color: active ? M.accent : M.text2, flexShrink: 0 }} />
      <span style={{ fontFamily: "'Barlow', sans-serif", fontSize: 14, fontWeight: active ? 600 : 400, color: active ? M.accent : M.text2, whiteSpace: 'nowrap' }}>
        {item.label}
      </span>
    </Link>
  );
...
export default function MemberShell({ children }) {
  ...
  const displayName = profile ? `${profile.first_name || ''} ${profile.last_name || ''}`.trim() : 'Member';

  return (
    <div style={{ minHeight: '100vh', background: M.bg, color: M.text, fontFamily: "'Barlow', sans-serif" }}>

      {/* ── Top navbar ── */}
      <header style={{ position: 'sticky', top: 0, zIndex: 40, height: 60, background: M.bg2, borderBottom: `1px solid ${M.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 12px 4px 4px', background: M.card, border: `1px solid ${M.border2}`, borderRadius: 10 }}>
            <div style={{ width: 28, height: 28, borderRadius: 7, background: M.accentbg2, border: `1.5px solid ${M.accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bebas Neue', sans-serif", fontSize: 11, fontWeight: 700, color: M.accent }}>
              {initials}
            </div>
            <span className="hidden sm:block" style={{ fontFamily: "'Barlow', sans-serif", fontSize: 13, fontWeight: 600, color: M.text, whiteSpace: 'nowrap' }}>
              {displayName}
            </span>
          </div>

          <button onClick={handleLogout} title="Sign out"
            style={{ width: 36, height: 36, borderRadius: 8, background: 'transparent', border: `1px solid ${M.border2}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: M.text2, cursor: 'pointer', transition: 'all .15s' }}
          ...
          <button onClick={() => setMobileOpen((p) => !p)}
            style={{ width: 36, height: 36, borderRadius: 8, background: 'transparent', border: `1px solid ${M.border2}`, display: isMobile ? 'flex' : 'none', alignItems: 'center', justifyContent: 'center', color: M.text2, cursor: 'pointer' }}
          >
      ...
      {mobileOpen && (
        <>
          ...
          <div style={{ position: 'fixed', top: 60, left: 0, right: 0, background: M.bg2, borderBottom: `1px solid ${M.border}`, padding: '12px 16px', zIndex: 50, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {NAV.map((item) => (
