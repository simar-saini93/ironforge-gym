  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px 0', color: M.text2, fontFamily: "'Barlow', sans-serif", fontSize: 14 }}>No payment records yet.</div>
      ) : (
        <>
            ...
            {payments.map((p, i) => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: i < payments.length - 1 ? `1px solid ${M.border}` : 'none', flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 24, letterSpacing: 1, color: M.green }}>{formatCurrency(p.amount)}</span>
      ...
      <ReceiptModal paymentId={receiptId} open={!!receiptId} onClose={() => setReceiptId(null)} role="member" />
    </div>
  );
}
