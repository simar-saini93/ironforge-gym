...
import { formatDate, formatCurrency, daysLeft, planLabel } from '@/lib/utils/format';
...
function SubCard({ sub, active }) {
  ...
  return (
    <div style={{ background: active ? M.accentbg2 : M.card, border: `1px solid ${active ? M.accent : M.border}`, borderRadius: 14, padding: '20px', position: 'relative', overflow: 'hidden' }}>
      {active && (
      ...
      <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, letterSpacing: 1, color: active ? M.accent : M.text, lineHeight: 1, marginBottom: 4 }}>{planLabel(sub.plan?.billing_cycle)}</p>
      <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: active ? M.accent : M.text2, marginBottom: 16 }}>{formatCurrency(sub.plan?.price)}</p>
...
export default function MemberSubscription() {
  ...
  useEffect(() => {
    fetch('/api/member/subscription')
      .then((r) => r.json())
      .then(({ subs }) => { setSubs(subs || []); setLoading(false); })
